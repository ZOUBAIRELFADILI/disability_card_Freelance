...
} & import("../api/adminApi").CarersApplication;

// Supporting Documents Section Component
const SupportingDocumentsSection: React.FC<{ supportingDocuments?: string[] }> = ({ supportingDocuments }) => {
  return supportingDocuments && supportingDocuments.length > 0 ? (
    <div className="space-y-3">
      {supportingDocuments.map((docPath: string, index: number) => {
        // Get file extension for download filename
        const extMatch = docPath.match(/\.[^/.]+$/);
...
const CarersPage: React.FC = () => {
  const [applications, setApplications] = useState<CarersApplicationWithDocs[]>([]);
  const [loading, setLoading] = useState(true);
  const { markAsViewed } = useNotifications();
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [selectedApplication, setSelectedApplication] = useState<CarersApplicationWithDocs | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [showCreateCardModal, setShowCreateCardModal] = useState(false);
  const [selectedApplicationForCard, setSelectedApplicationForCard] = useState<CarersApplicationWithDocs | null>(null);
  ...
  const handleCreateCard = (application: CarersApplicationWithDocs) => {
  ...
  const ApplicationModal = ({ application }: { application: CarersApplicationWithDocs }) => (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/95 backdrop-blur-md rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-white/20">
        <div className="sticky top-0 bg-gradient-to-r from-pink-50 to-white border-b px-8 py-5 flex justify-between items-center rounded-t-2xl">
          <h2 className="text-2xl font-bold text-pink-800 tracking-tight flex items-center gap-2">
            <CreditCard className="w-6 h-6 text-pink-600" /> Carers Card Application Details
          </h2>
          <button
        ...
        <div className="p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {/* Left: Profile & Supporting Documents */}
            <div className="space-y-8">
              ...
              <div className="bg-gray-50 rounded-xl p-6 shadow-sm border">
                <h3 className="text-lg font-semibold mb-4 text-pink-800 flex items-center gap-2">
                  <CreditCard className="w-5 h-5 text-pink-600" /> Supporting Documents
                </h3>
                <SupportingDocumentsSection supportingDocuments={application.supportingDocuments} />
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border space-y-3">
                <h3 className="text-lg font-semibold mb-4 text-pink-800">Personal Information</h3>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border space-y-3">
                <h3 className="text-lg font-semibold mb-4 text-pink-800">Caregiving Information</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-gray-500">Care Recipient Name</label>
                    <p className="text-gray-900">{application.careRecipientName}</p>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-500">Relationship to Recipient</label>
                    <p className="text-gray-900">{application.relationshipToRecipient}</p>
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-gray-500">Caregiving Experience</label>
              ...
              {/* Application Status */}
              <div className="bg-white rounded-xl p-6 shadow-sm border space-y-3">
                <h3 className="text-lg font-semibold mb-4 text-pink-800">Application Status</h3>
                  ...
                  <div className="space-y-3 pt-2 border-t">
                    ...
                    <div className="flex space-x-3">
                      <button
                        ...
                        className="flex items-center px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 text-sm"
                      >
